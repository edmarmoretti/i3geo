...
i3GEOF.metaestat = {
	...
	inicia: function(iddiv){
		i3GEOF.metaestat.iniciaDicionario();
	},
	...
	adicionaCamada: function(){
		...
		var v = i3GEOF.metaestat.verificaParametros(),
			temp = function(retorno){
				if(i3GEO.arvoreDeCamadas.pegaTema(retorno.layer) == ""){
					i3GEOF.metaestat.desligaCamadas();
		...
		i3GEO.php.mapfileMedidaVariavel(
			...
			i3GEOF.metaestat.defineFiltro(),
			0,
			$i("i3geoCartoComboTipoRep").value,
			i3GEOF.metaestat.defineTitulo(),
			$i("i3geoCartoComboTipoClassificacao").value,
			i3GEOF.metaestat.defineAgruparPor()
		);
	},
	...
	principal: {
		inicia: function(iddiv){
			...
			i3GEOF.metaestat.principal.comboVariaveis();
		},
		abreJanela: function(){
		...
		maisInfo: function(){
			...
			if(!v || v.value === ""){
				...
				janela.setBody(i3GEOF.metaestat.aguarde());
				temp = function(retorno){
		...
		comboVariaveis: function(){
			var onde = $i("i3geoCartoVariaveis"),
				temp = function(dados){
					...
					ins = '<p style="text-align:left;"><input type="button" id="i3GEOcartoBotaoInfo" value="'+$trad(3,i3GEOF.metaestat.dicionario)+'"class="paragrafo" style="width:200px;cursor:pointer;color:blue" /></p>' +
						...
						"<select id='i3geoCartoComboVariavel' style='box-shadow:0 1px 5px gray;width:"+(i3GEOF.metaestat.LARGURA - 20)+"px' onchange='i3GEOF.metaestat.principal.comboVariaveisOnchange(this)'><option value=''>---</option>";
					for(i=0;i<n;i++){
					...
					if(onde){
						...
						$i("i3GEOcartoBotaoInfo-button").style.width = i3GEOF.metaestat.LARGURA - 25 + "px";
						new YAHOO.widget.Button(
		...
		comboVariaveisOnchange: function(combo){
			if(combo.value != ""){
				i3GEOF.metaestat.principal.comboMedidasVariavel(combo.value);
			}
			else{
		...
		comboMedidasVariavel: function(codigo_variavel){
			var onde = $i("i3geoCartoMedidasVariavel"),
				temp = function(dados){
					var n = dados.length,
						ins = '<p class="paragrafo" >'+$trad(2,i3GEOF.metaestat.dicionario)+'</p>',
						i;
					ins += "<select id='i3geoCartoComboMedidasVariavel' style='width:"+(i3GEOF.metaestat.LARGURA - 20)+"px' onchange='i3GEOF.metaestat.principal.comboMedidaVariavelOnchange(this)'><option value=''>---</option>";
					for(i=0;i<n;i++){
		...
		comboTipoRep: function(){
			...
			if(onde){
				...
				new YAHOO.widget.Button(
					...
					{onclick:{fn: i3GEOF.metaestat.adicionaCamada}}
				);
