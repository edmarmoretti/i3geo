...
{$i("guia2obj").style.display="block";limpalista();selecionarmarcadosf()}
$i("guia3").onclick = function()
{
 	$i("guia3obj").style.display="block";
 	comboitens("estatItem",tema,function(retorno)
	{$i("itensGuia3").innerHTML = retorno})
}
$i("guia4").onclick = function()
{$i("guia4obj").style.display="block";t0();}
...
comboCamadas("selCamada",tema,function(retorno)
{
	if (retorno == "") //tema unico
 	...
 	{
		$i("combot").innerHTML = "<div style=top:10px class=destaca >O tema escolhido &eacute; composto por mais de uma camada de dados. Selecione uma das camadas para listar:</div><br>"+retorno
...
function selecionarmarcadosf()
{
	...
	$i("guia1").style.backgroundColor="rgb(230,230,230)"
	$i("guia2").style.backgroundColor="rgb(255,255,255)"
	aguarde("none")
}
...
function ativaself()
{
	...
	cp.call(p,"incluiSel",window.parent.ajaxredesenha);
}
...
function t2a()
{
 	...
 	else
 	{
		...
		{
	 		combo1 = retorno
	 		combo2 = retorno.replace("gi1","gi2")
	 		t2()
		})
	}
}
