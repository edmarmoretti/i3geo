                    ...
                    i3GEO.request.get({
                        ...
                        fn: function (data) {
                            var linhas = [
                                {
                                    "nome": "GeoHash " + gh,
                                    "click": "window.open('https://geohash.softeng.co/" + gh + "')"
                                }
                            ];
                            for (ig = 0; ig < data.length; ig++) {
                                if (data[ig].PUBLICADO && data[ig].PUBLICADO.toLowerCase() == "sim") {
                                    sistema = data[ig].NOME;
                                    exec = data[ig].ABRIR;
                                    exec = exec.replace("{{x}}", p.x);
                                    exec = exec.replace("{{y}}", p.y);
                                    t = "blank";
                                    if (data[ig].TARGET) {
                                        t = data[ig].TARGET;
                                    }
