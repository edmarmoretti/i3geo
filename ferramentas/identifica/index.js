...
function mostraf(retorno)
{
	...
	if (retorno.data != undefined)
	{
		...
		for (octemasc=0;octemasc<octemas.length;octemasc++)
		{
			...
			res += "<div style='left:2px;text-align:left;background-color:white;width:100%' >"+titulo[0]+"</div>"
			//verifica se é WS
			...
			for (oc=0;oc<ocs.length;oc++)
			{
				if (ocs[oc] != "")
				{
					...
					for (par=0;par<pares.length;par++)
					{
						...
						{res = res + "<div style='width:100%;text-align:left;background-color:"+cor+"' >&nbsp;&nbsp;" + valores[0] + " <a href='" + vlink + "' > link</a></div>"}
						else if ((valores[2] == " ") || (valores[2] != undefined))
						{
							var testaIcone = (valores[0].split(".png")).length
							if (testaIcone == 1) //nao é do tipo ícone
							{res = res + "<div style='border-top:0px solid brown;font-size:9px;width:100%;text-align:left;background-color:"+cor+"' ><b>&nbsp;&nbsp;" + valores[0] + " </b>" + valores[1] + "</div>"}
							else //corrige o caminho do ícone
							{
								var i = valores[0].replace("..","../..")
								res = res + "<div style='width:100%;text-align:left;background-color:"+cor+"' >" + i + " <i>" + valores[1] + "</i></div>"
							}
						}
						contav = contav + 1
