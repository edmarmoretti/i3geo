...
$i("guia2").onclick = function()
{
	...
	comboCamadas("selCamada",tema,function(retorno)
	{
	 	if (retorno != "")
		{
		 	$i("combot").innerHTML = "<span style=background-color:yellow >O tema escolhido &eacute; composto por mais de uma camada de dados. Selecione uma das camadas para gerar a legenda nova:</span>"+retorno
			$i("selCamada").onchange = function()
			{
				tema = $i("selCamada").value
				comboitens("selItem",$i("selCamada").value,function(retorno)
		 		{$i("itens").innerHTML = retorno})
			}
		}
		else
		{
...
function valorU()
{
	...
	cp.call(p,"alteraclasse",window.parent.ajaxredesenha);
}
function simbU()
{
	aguarde("block")
...
function representacao()
{
	...
	window.parent.g_operacao = "outras"
	//window.parent.borra("sim");
	cp.call(p,"alteraRepresentacao",window.parent.ajaxredesenha);
}
...
function aplicaEstilo()
{
	...
	cp.set_response_type("JSON");
	window.parent.g_operacao = "outras"
	cp.call(p,"editasimbolo",window.parent.ajaxredesenha);
}
//exclui estilo
