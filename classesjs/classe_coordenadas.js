...
i3GEO.coordenadas = {
	...
	parametro definido no mapfile de inicializa&ccedil;&atilde;o do mapa (veja em i3geo/aplicmap/geral1.map ou geral1windows.map)
	...
	Nas vers&otilde;es anteriores a 4.5 do i3Geo utilizava-se um
	...
	prefixo {string} - prefixo da tabela (veja criaMascaraDMS)
		...
		atualizaGeo: function(dmsx,dmsy,prefixo){
			...
			i3GEO.coordenadas.MODOTEXTO += "DMS - Latitude: "+y[0]+" "+y[1]+" "+y[2]+" Longitude: "+x[0]+" "+x[1]+" "+x[2]+"<br><br>";
		},
		/*
	...
	{string} - html com a tabela
		...
		caixa {string} - (opcional) caixa de sele&ccedil;&atilde;o de tipos
		...
		criaMascaraCodigo: function(prefixo,titulo,caixa,tipoCodigo){
			...
			ins = "<table id="+prefixo+" class='i3GeoMascaraCoord' ><tr>" +
			"<td>"+caixa+"&nbsp;<td>" +
			"<td style=width:100px;text-align:right >"+titulo+" :&nbsp;</td>" +
			"<td>"+$inputText("","",prefixo+"Codigo","Cod","12","00")+"&nbsp;</td>" +
			'<td><img class=tic title=zoom onclick="'+funcao+'" src="'+i3GEO.util.$im("branco.gif")+'" /></td>' +
			"</tr></table>";
			return ins;
		},
		/*
				...
				for(i=0;i<n;i += 1){
					...
					if(temp.ativo === true){
						if(temp.tipo === "geo"){
							...
							if(ativaMovimento === true){
								if(i3GEO.Interface.TABLET === true){
									if(i3GEO.eventos.MOUSECLIQUE.toString().search(nomeFunc+"('"+onde+"','"+tipos[i]+"')") < 0){
										i3GEO.eventos.MOUSECLIQUE.push(nomeFunc+"('"+onde+"','"+tipos[i]+"')");
									}
								}
								else{
									if(i3GEO.eventos.MOUSEMOVE.toString().search(nomeFunc+"('"+onde+"','"+tipos[i]+"')") < 0){
										i3GEO.eventos.MOUSEMOVE.push(nomeFunc+"('"+onde+"','"+tipos[i]+"')");
									}
								}
							}
							if(typeof(x) !== 'undefined'){
