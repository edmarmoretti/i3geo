...
i3GEO.coordenadas =
	{
		 ...
		 * Valores:
		 *
		 * bloco - mostra apenas um dos tipos e uma caixa de
		 * sele&ccedil;&atilde;o
		 *
		 * separado - mostra todos os tipos em lugares diferentes conforme o
		 * valor de idhtml
		 *
		 * lista - mostra cada tipo em um lugar diferente conforme o valor de
		 * idhtml
		 *
		 ...
		 * Propriedade: padrao
		 *
		 * Indica qual tipo de coordenada &eacute; mostrado como padr&atilde;o
		 * quando formato for igual a "bloco".
		 *
		 ...
		 *
		 * CRS que define a proje&ccedil;&atilde;o original das coordenadas
		 * capturadas na tela. Deve ser o mesmo parametro definido no mapfile de
		 * inicializa&ccedil;&atilde;o do mapa (veja em
		 * i3geo/aplicmap/geral1.map ou geral1windows.map)
		 *
		 * Type:
		 *
		 * {String}
		 ...
		 * Propriedade: config
		 *
		 * Define as configura&ccedil;&otilde;es de cada tipo de coordenada que
		 * ser&aacute; utilizada e/ou mostrada no mapa
		 *
		 * Para alterar os parametros ou acrescentar novas
		 * proje&ccedil;&otilde;es, altere esse objeto
		 *
		 * Para mais detalhes, veja i3geo/classesjs/classe_coordenada.js
		 *
		 * Para desativar a apresenta&ccedil;&atilde;o de uma
		 * proje&ccedil;&atilde;o altere o valor de "ativo".
		 *
		 * Exemplo:
		 *
		 * (start code)
		 ...
		 *
		 * Essa vari&aacute;vel define os parametros individuais de cada
		 * componente que pode ser utilizado no mapa.
		 *
		 ...
		 *
		 * Obt&eacute;m e mostra as coordenadas Geogr&aacute;ficas da
		 * posi&ccedil;&atilde;o do mouse sobre o mapa.
		 *
		 * Se voc&ecirc; n&atilde;o quer essa fun&ccedil;&atilde;o no mapa,
		 * elimine o elemento HTML que contenha o id definido em
		 * i3GEO.coordenadas.PARAMETROS, por default &eacute; "localizarxy"
		 *
		 * Parametro:
		 *
		 * {String} - id do elemento HTML que receber&aacute; o resultado.
		 * Esse id por default &eacute; obtido de i3GEO.coordenadas.PARAMETROS
		 */
		mostraCoordenadasGEO : function(id) {
			...
			try {
				...
				i3GEO.janela.tempoMsg("mostraCoordenadasGeo: "
					+ e.description);
			}
		},
		/**
		 ...
		 *
		 * Determina qual &eacute; a zona UTM de um par de coordenadas
		 * geogr&aacute;ficas
		 *
		 * Parametros:
		 *
		 ...
		 *
		 * Cria uma tabela para mostrar as coordenadas no padr&atilde;o grau
		 * minuto e segundo
		 *
		 ...
		 * Parametro:
		 *
		 * {String} - prefixo para batizar os ids dos elementos que
		 * ser&atilde;o criados
		 *
		 * Retorno:
		 *
		...
		criaMascaraDMS : function(prefixo, titulo, caixa) {
			var ins = '<table class="i3GeoMascaraCoord" id='
				+ prefixo
				+ ' ><tr>'
				+ "<td>"
				+ caixa
		 ...
		 *
		 * Caso o elemento HTML com id igual a prefixo + "xg" n&atilde;o existir,
		 * apenas i3GEO.coordenadas.MODOTEXTO ser&aacute; atualizado
		 *
		 * Parametros:
		 *
		...
		atualizaGeo : function(dmsx, dmsy, prefixo) {
			...
			i3GEO.coordenadas.MODOTEXTO += "DMS - Latitude: "
				+ y[0]
				+ " "
				+ y[1]
				+ " "
		 ...
		 *
		 * Cria uma tabela para mostrar as coordenadas no padr&atilde;o
		 * m&eacute;trico (x e y)
		 *
		 ...
		 *
		 * Cria uma tabela para mostrar as coordenadas no padr&atilde;o de
		 * codigo, como no caso de geohash
		 *
		 * A tabela criada receber&aacute; o id prefixo+"tabela"
		 *
		 * Parametro:
		...
		criaMascaraCodigo : function(prefixo, titulo, caixa, tipoCodigo) {
			...
			if (tipoCodigo === "geohash") {
				funcao = "i3GEO.coordenadas.geohash.zoomCodigo(\'"
					+ prefixo
					+ "Codigo\')";
			}
			ins = "<table id="
		...
		atualizaCodigo : function(onde, configProj, x, y) {
			...
			if (temp.tipoCodigo === "geohash") {
				codigo = i3GEO.coordenadas.geohash.encodeGeoHash(
					y,
					x);
			}
			i3GEO.coordenadas.MODOTEXTO += temp.titulo
				+ " : "
				+ codigo
				+ "<br><br>";
			if (ondeValor) {
		...
		/**
		 * Atualiza os valores em uma tabela do tipo x e y com base na
		 * biblioteca Proj4
		 *
		...
		atualizaProj4 : function(onde, configProj, x, y) {
			...
			if (temp.defepsg === ""
				...
				p = i3GEO.coordenadas.calculaProj4(
					i3GEO.coordenadas.defOrigem,
					destino,
					x,
					y);
			}
		 ...
		 * Parametros:
		 *
		 * {string} - CRS contendo o c&oacute;digo da
		 * proje&ccedil;&atilde;o de origem
		 *
		 * {string} - CRS contendo o c&oacute;digo da
		 * proje&ccedil;&atilde;o de destino
		 *
		...
		calculaProj4 : function(origem, destino, x, y) {
			...
			var source = new Proj4js.Proj(
				"ORIGEM"), dest = new Proj4js.Proj(
				"DESTINO"), p = new Proj4js.Point(
				x, y);
			Proj4js.transform(
				source,
				dest,
		...
		/**
		 * Muda o tipo de coordenada que est&aacute; sendo mostrada no formato
		 * "bloco".
		 *
		 * Parametro:
		 *
		...
		/**
		 * Constr&oacute;i o conjunto de elementos HTML para mostrar as
		 * coordenadas e define as fun&ccedil;&otilde;es de
		 * atualiza&ccedil;&atilde;o.
		 *
		 * Parametro:
		 *
		 * {boolean} - (opcional) aplica ou n&atilde;o as
		 * fun&ccedil;&otilde;es ligadas a movimenta&ccedil;&atilde;o do mouse
		 *
		 * {string} - (opcional) id onde o resultado ser&aacute; mostrado
		 * (ir&aacute; ignorar os ids definidos em coordenadas.config)
		 */
		mostraCoordenadas : function(ativaMovimento, onde, x, y) {
			try {
				...
				caixa = "<select onchange='javascript:i3GEO.coordenadas.mudaTipo(this,\""
					+ onde
				...
				for (i = 0; i < n; i += 1) {
					...
					if (temp.ativo === true) {
						if (temp.tipo === "geo") {
							ins += i3GEO.coordenadas.criaMascaraDMS(
								onde
									+ tipos[i],
								temp.titulo,
								caixa);
							...
							if (temp.tipo === "codigo") {
								ins += i3GEO.coordenadas.criaMascaraCodigo(
									onde
										+ tipos[i],
									temp.titulo,
									caixa,
								...
								ins += i3GEO.coordenadas.criaMascaraMetrica(
									onde
										+ tipos[i],
									temp.titulo,
									caixa);
				...
				if (this.formato === "janela") {
					...
					YAHOO.util.Event.addListener(
						...
						"click",
						function() {
								i3GEO.coordenadas.formato = "bloco",
								...
								if (i3GEO.Interface.TABLET === true) {
									if (i3GEO.eventos.MOUSECLIQUE.toString().search(
										nomeFunc
											...
											+ tipos[i]
											+ "')") < 0) {
										i3GEO.eventos.MOUSECLIQUE.push(nomeFunc
											+ "('"
											+ onde
											+ "','"
											+ tipos[i]
									...
									if (i3GEO.eventos.MOUSEMOVE.toString().search(
										nomeFunc
											...
											+ tipos[i]
											+ "')") < 0) {
										i3GEO.eventos.MOUSEMOVE.push(nomeFunc
											+ "('"
											+ onde
											+ "','"
											+ tipos[i]
				...
				if (ativaMovimento === true) {
					if (i3GEO.Interface.TABLET === true) {
						if (i3GEO.eventos.MOUSECLIQUE.toString().search(
							"i3GEO.coordenadas.limpaModoTexto()") < 0) {
			...
			decodeGeoHash : function(geohash) {
				...
				for (i = 0; i < geohash.length; i++) {
					...
					for (j = 0; j < 5; j++) {
						...
						if (is_even) {
							lon_err /= 2;
							i3GEO.coordenadas.geohash.refine_interval(
								lon,
								cd,
								mask);
						} else {
