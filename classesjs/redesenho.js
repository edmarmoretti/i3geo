	...
	if ((retorno.data != "erro") && (retorno.data != undefined))
	{
		...
		$i("legenda").onclick = function()
		{
			...
			if (!$i("moveLegi"))
			{
				...
				temp += '<div class="tr"></div></div><div id=wlegenda style=text-align:left >';
				...
				var escondeLeg = function()
				{
					YAHOO.util.Event.removeListener(YAHOO.moveLegi.xp.panel.close, "click");
					YAHOO.moveLegi.xp.panel.destroy();	
				}
				YAHOO.moveLegi.xp.panel.render();
				YAHOO.moveLegi.xp.panel.show();
				YAHOO.util.Event.addListener(YAHOO.moveLegi.xp.panel.close, "click", escondeLeg);
			}
		}
	}
}
...
function ajaxredesenha(retorno)
{
	...
	if ((retorno != "erro") && (retorno != undefined))
	{
		if (retorno.search("var mapimagem=") > -1){ajaxIniciaParametros(original);}
		else
		{
