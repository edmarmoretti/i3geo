...
i3GEO.Interface =
	{
		...
		openlayers : {
			...
			balao : function(texto, completo, x, y) {
				...
				texto = c + texto;
				b = new OpenLayers.Popup.FramedCloud(
					null, 
					i3GEO.util.projGeo2OSM(
				...
				//e.style.right = "13px";
				//e.style.top = parseInt(z.style.top, 10) + 20 + "px";
				//e.style.zIndex = 1;
				...
				//$i(b.id + "_GroupDiv").appendChild(e);
			...
			adicionaNoArvoreKml : function(url, nomeOverlay, ativo, id) {
				...
				html =
					"<input onclick='i3GEO.Interface.openlayers.ativaDesativaCamadaKml(this,\"" + url
						+ "\")' class=inputsb style='cursor:pointer;' type='checkbox' value='" + id + "'";
				if (ativo === true) {
					html += " checked ";
				}
				html += "/>";
			...
			criaLayers : function() {
				var configura = i3GEO.configura, url =
					configura.locaplic + "/classesphp/mapa_openlayers.php?g_sid=" + i3GEO.configura.sid + "&TIPOIMAGEM="
				...
				if (i3GEO.Interface.openlayers.googleLike === true) {
					url =
						configura.locaplic + "/classesphp/mapa_googlemaps.php?g_sid=" + i3GEO.configura.sid + "&TIPOIMAGEM="
					...
					if (i3geoOL.getLayersByName(camada.name).length === 0 && camada.name.toLowerCase() != "copyright") {
						...
						if (camada.plugini3geo && camada.plugini3geo != "") {
							...
							try {
								...
								// &&
								if (i3GEO.Interface.openlayers.googleLike === false && camada.connectiontype === 7 && camada.wmsurl !== ""
									...
									if (camada.tiles === "nao" || camada.escondido.toLowerCase() === "sim" || camada.connectiontype === 10
										|| (camada.type === 0 && camada.cache === "nao") || camada.type === 8) {
			...
			ligaDesliga : function(obj) {
				...
				b = new Image();
				b.src =
					i3GEO.configura.locaplic + "/classesphp/mapa_controle.php?funcao=ligatemasbeacon&desligar=" + desligar + "&ligar="
						+ ligar + "&adicionar=nao&g_sid=" + i3GEO.configura.sid;
				b.onerror = function() {
		...
		googlemaps : {
			...
			balao : function(texto,completo, x, y) {
				...
				/*
				 * google.maps.event.addListenerOnce(b,'domready',function(){ var p,c,a = $i(e); p = i3GEO.Interface.googlemaps.BALAOPROP; c =
				 * a.parentNode.parentNode.parentNode.parentNode; google.maps.event.addDomListener(a.parentNode.parentNode.parentNode,
				 * 'click', function (evt){ objposicaocursor.ddx = null; } ); e = document.createElement("img"); e.id = a.id + "_fixa";
				 * e.style.width = "16px"; e.style.height = "16px"; e.style.position = "absolute"; e.style.right = "13px"; e.style.top =
				 * "30px"; e.style.zIndex = 1; e.style.display = "block"; e.src = i3GEO.configura.locaplic + '/imagens/branco.gif';
				 * e.className = p.classeCadeado; //e.innerHTML = "<img style='width:16px;height:16px;' src='../imagens/branco.gif' />";
				 * e.onclick = function() { if (p.classeCadeado === "i3GEOiconeAberto") { p.classeCadeado = "i3GEOiconeFechado"; } else {
				 * p.classeCadeado = "i3GEOiconeAberto"; } e.className = p.classeCadeado; p.removeAoAdicionar = !p.removeAoAdicionar; return
				 * false; }; c.appendChild(e); //YAHOO.util.Event.addListener(c, "click", YAHOO.util.Event.stopPropagation);
			...
			cria : function(w, h) {
				...
				if (i) {
					...
					if (!f) {
						ins =
							'<div id=googlemapsdiv style="width:0px;height:0px;text-align:left;background-image:url('
								+ i3GEO.configura.locaplic + '/imagens/i3geo1bw.jpg)"></div>';
						i.innerHTML = ins;
					}
					f = $i("googlemapsdiv");
					if (w) {
		...
		googleearth : {
			...
			insereLayer : function(nomeLayer) {
				var kmlUrl =
					i3GEO.configura.locaplic + "/classesphp/mapa_googleearth.php?REQUEST=GetKml&g_sid=" + i3GEO.configura.sid + "&layer="
						+ nomeLayer + i3GEO.Interface.googleearth.PARAMETROSLAYER + "&r=" + Math.random(), linki3geo =
					i3GeoMap.createLink(''), nl = i3GeoMap.createNetworkLink('');
			...
			adicionaNoArvoreGoogle : function(url, nomeOverlay, ativo, id) {
				...
				html =
					"<input onclick='i3GEO.Interface.googleearth.ativaDesativaCamadaKml(this)' class=inputsb style='cursor:pointer;' type='checkbox' value='"
						+ id + "'";
				if (ativo === true) {
					html += " checked ";
				}
