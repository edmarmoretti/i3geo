...
i3GEO.Interface = {
	...
	aplicaOpacidade : function(opacidade, layer) {
		i3GEO.Interface[i3GEO.Interface.ATUAL].aplicaOpacidade(
			opacidade,
			layer);
	},
	/**
	 ...
	 * Parametros:
	 *
	 * {Objeto} -parametros obtidos da fun&ccedil;&atilde;o PHP de redesenho do
	 * mapa. Quando igual a "", &eacute; feita apenas a
	 * atualiza&ccedil;&atilde;o da camada, sem que a &aacute;rvore de camadas
	 * seja atualizada.
	 *
	 * {string} - c&oacute;digo do tema
	 */
	atualizaTema : function(retorno, tema) {
		i3GEO.Interface[i3GEO.Interface.ATUAL].atualizaTema(
			retorno,
			tema);
	},
	/**
	 ...
	 * ou objeto que contenha as propriedades value e checked, sendo value o
	 * c&oacute;digo do layer
	 */
	 ...
	 */
	cria : function(w, h) {
		i3GEO.Interface[i3GEO.Interface.ATUAL].cria(
			w,
			h);
	},
	/**
	...
	alteraParametroLayers : function(parametro, valor) {
		i3GEO.Interface[i3GEO.Interface.ATUAL].alteraParametroLayers(
			parametro,
			valor);
	},
	/**
	...
	openlayers : {
		 ...
		 * Default:
		 *
		 * {PanZoomBar:true,PanZoom:
		 * false,LayerSwitcher:true,ScaleLine:true,OverviewMap:false}
		 */
		GADGETS : {
			PanZoomBar : true,
			PanZoom : false,
		 ...
		 *
		 * Objetos do tipo LAYER que ser&atilde;o adicionados ap&oacute;s a
		 * crioa&ccedil;&atilde;o de todos os layers default.
		 *
		 * Tipo:
		 *
		 * {array}
		 ...
		 *
		 * Indica se a proje&ccedil;&atilde;o cartogr&aacute;fica do mapa atual
		 * &eacute; a mesma utilizada pela API do Google Maps
		 *
		 * Tipo:
		 *
		 * {boolean}
		...
		balao : function(texto, x, y) {
			var e, z, p = i3GEO.Interface.openlayers.BALAOPROP, b = new OpenLayers.Popup.FramedCloud(
				null, i3GEO.util.projGeo2OSM(new OpenLayers.LonLat(
					x, y)), null, texto, null, true);
			b.maxsize = new OpenLayers.Size(
				parseInt(
					i3GEO.parametros.w / 2,
					10), parseInt(
			...
			$i(
				b.id
					+ "_GroupDiv").appendChild(
				e);
		},
		/**
		 * Redesenha o mapa atual
		 *
		 * s&atilde;o criados apenas os layers que ainda n&atilde;o existirem no
		 * mapa mas que existem na arvore de camadas
		 */
		redesenha : function() {
		...
		/**
		 * Cria o mapa do lado do cliente (navegador) Define o que for
		 * necessario para a criacao de
		 *
		...
		cria : function(w, h) {
			...
			i3geoOL = new OpenLayers.Map(
				'openlayers', i3GEO.Interface.openlayers.parametrosMap);
		},
		/**
		...
		inicia : function() {
			...
			var montaMapa = function() {
				...
				if (openlayers.GADGETS.ScaleLine === true) {
					...
					i3geoOL.addControl(pz);
					pz.div.style.left = i3GEO.Interface.BARRADEZOOMLEFT
						+ 5
						+ "px";
				}
			...
			if (i3GEO.parametros.kmlurl !== "") {
				i3GEO.Interface.openlayers.adicionaKml(
					true,
		...
		/**
		 * Adiciona ao mapa o componente com a lista de arquivos KML que o
		...
		adicionaKml : function(pan, url, titulo, ativo) {
			...
			i3GEO.Interface.openlayers.adicionaNoArvoreKml(
				url,
				titulo,
				ativo,
				ngeoxml);
		...
		criaArvoreKML : function() {
			...
			node = new YAHOO.widget.HTMLNode(
				d, root, true, true);
		...
		adicionaNoArvoreKml : function(url, nomeOverlay, ativo, id) {
			...
			node = i3GEO.Interface.openlayers.ARVORE.getNodeByProperty(
				"idkml",
				"raiz");
			...
			nodekml = new YAHOO.widget.HTMLNode(
				d, node, true, true);
			...
			node.expand();
			if (ativo === true) {
				i3GEO.Interface.openlayers.insereLayerKml(
					id,
					url);
			}
		},
		...
		/**
		 * Cria os layers conforme os parametros definidos em
		 * i3GEO.arvoreDeCamadas.CAMADAS
		 */
		criaLayers : function() {
			...
			if (i3geoOL.getLayersByName("Nenhum").length === 0
				...
				if ($i(i3geoOL.id
					+ "_OpenLayers_ViewPort")) {
					$i(i3geoOL.id
						+ "_OpenLayers_ViewPort").style.backgroundColor = "rgb("
		...
		/**
		 * Altera a posicao do layer de desenho de figuras, posicionando-o sobre
		 * todos os demais
		 */
			...
			if ($i("ArvoreTituloTema"
				...
				YAHOO.util.Dom.setStyle(
					"ArvoreTituloTema"
						+ event.object.name,
					"background",
					i3GEO.Interface.openlayers.FUNDOTEMA);
				if (p) {
					p.style.display = "block";
					i3GEO.arvoreDeCamadas.progressBar.set(
						'maxValue',
		...
		ligaDesliga : function(obj) {
			...
			if (obj.checked) {
				ligar = obj.value;
				i3GEO.arvoreDeCamadas.alteraPropCamadas(
					"status",
					"2",
					obj.value);
			} else {
		...
		registraEventos : function() {
			...
			calcCoord = function(e) {
				...
				if (i3GEO.Interface.openlayers.googleLike === true) {
					projWGS84 = new OpenLayers.Projection(
						"EPSG:4326");
					proj900913 = new OpenLayers.Projection(
						"EPSG:900913");
			...
			//
			i3GEO.eventos.ativa($i(i3geoOL.id
				+ "_OpenLayers_Container"));
			//
		...
		ativaBotoes : function() {
			...
			if ($i("barraDeBotoes2")
				...
				x2 = imagemxy[0]
					+ i3GEO.Interface.BARRABOTOESLEFT;
				y2 = imagemxy[1]
					+ i3GEO.Interface.BARRABOTOESTOP;
			}
			if ($i("barraDeBotoes2")
				|| i3GEO.barraDeBotoes.AUTO === true) {
				i3GEO.barraDeBotoes.inicializaBarra(
		...
		/**
		 * Recalcula parametros do mapa atual sempre que ocorre alteracao na
		 * extensao geografica
		 */
		recalcPar : function() {
			...
			i3GEO.parametros.mapexten = bounds[0]
				+ " "
				+ bounds[1]
				+ " "
				+ bounds[2]
			...
			i3GEO.gadgets.atualizaEscalaNumerica(parseInt(
				escalaAtual,
				10));
		},
		...
		pan2ponto : function(x, y) {
			...
			i3geoOL.panTo(new OpenLayers.LonLat(
				x, y));
		}
	},
	/**
	...
	googlemaps : {
		...
		balao : function(texto, x, y) {
			...
			p.baloes.push(b);
			//TODO resolver icone do cadeado
			/*
			google.maps.event.addListenerOnce(b,'domready',function(){
				...
				e.style.zIndex = 1;
				e.style.display = "block";
				e.src = i3GEO.configura.locaplic + '/imagens/branco.gif';
				e.className = p.classeCadeado;
				//e.innerHTML = "<img style='width:16px;height:16px;' src='../imagens/branco.gif' />";
				e.onclick = function() {
					if (p.classeCadeado === "i3GEOiconeAberto") {
						p.classeCadeado = "i3GEOiconeFechado";
					} else {
		...
		atualizaTema : function(retorno, tema) {
			...
			i3GEO.Interface.googlemaps.insereLayer(
				tema,
				indice);
			if (retorno === "") {
				return;
		...
		inicia : function() {
			...
			montaMapa = function(retorno) {
				...
				try {
					i3GeoMap = new google.maps.Map(
						$i(i3GEO.Interface.IDMAPA), i3GEO.Interface.googlemaps.MAPOPTIONS);
				} catch (e) {
					alert(e);
					return;
				}
				...
				if (i3GEO.parametros.kmlurl !== "") {
					i3GEO.Interface.googlemaps.adicionaKml(
						true,
						i3GEO.parametros.kmlurl);
				}
				// executa fun&ccedil;&atilde;o de finaliza&ccedil;&atilde;o, se
		...
		insereLayer : function(nomeLayer, indice, cache) {
			if (i3GEO.pluginI3geo.existeObjeto(nomeLayer) === false) {
				var i = i3GEO.Interface.googlemaps.criaImageMap(
					nomeLayer,
					cache);
				i3GeoMap.overlayMapTypes.insertAt(
					indice,
					i);
			}
		},
		registraEventos : function() {
			...
			google.maps.event.addListener(
				...
				function(ponto) {
					...
					ponto = ponto.latLng;
					teladms = i3GEO.calculo.dd2dms(
						ponto.lng(),
						ponto.lat());
					tela = i3GeoMapOverlay.getProjection().fromLatLngToContainerPixel(
						ponto);
					objposicaocursor = {
						ddx : ponto.lng(),
						ddy : ponto.lat(),
		...
		ligaDesliga : function(obj) {
			...
			if (obj.checked
				&& (!indice || plugin === true)) { // layer nao existe mas foi
				// ligado
				ligar = obj.value;
				...
				for (i = 0; i < n; i++) {
					if (i3GEO.util.in_array(
						i3GEO.arvoreDeCamadas.CAMADAS[i].name,
						listatemp)) {
		...
		recalcPar : function() {
			try {
				...
				i3GEO.parametros.mapexten = sw.lng()
					+ " "
					+ sw.lat()
					+ " "
					+ ne.lng()
				...
				sw = i3GeoMapOverlay.getProjection().fromContainerPixelToLatLng(
					new google.maps.Point(
						0, 1));
				ne = i3GeoMapOverlay.getProjection().fromContainerPixelToLatLng(
					new google.maps.Point(
						1, 0));
				i3GEO.parametros.pixelsize = sw.lng()
					- ne.lng();
		...
		zoom2extent : function(mapexten) {
			var re = new RegExp(
				",", "g"), pol = mapexten.replace(
				re,
				" "), ret = pol.split(" "), sw = new google.maps.LatLng(
				ret[1], ret[0]), ne = new google.maps.LatLng(
				ret[3], ret[2]);
			i3GeoMap.fitBounds(new google.maps.LatLngBounds(
				sw, ne));
		...
		pan2ponto : function(x, y) {
			i3GeoMap.panTo(new google.maps.LatLng(
				y, x));
		},
		/**
		 * Function: adicionaKml
		 ...
		 * Parametros:
		 *
		 * {Boolean} - define se o mapa ser&aacute; deslocado para encaixar o
		 * KML
		 *
		 * {String} - URL do arquivo KML. Se n&atilde;o for definido, a URL
		 * ser&aacute; obtida do INPUT com id = i3geo_urlkml (veja
		 * i3GEO.gadgets.mostraInserirKml)
		 *
		 * {string} - titulo que aparecer&aacute; na &aacute;rvore. Se
		 * n&atilde;o for definido, ser&aacute; calculado aleatoriamente.
		 *
		 * {boolean} - indica se a camada estar&aacute; ativa ou n&atilde;o. Se
		 * n&atilde;o for definido, ser&aacute; considerado como true
		 */
		adicionaKml : function(pan, url, titulo, ativo) {
			var ngeoxml, i;
			if (!$i("arvoreCamadasKml")) {
			...
			i3GEO.Interface.googlemaps.adicionaNoArvoreGoogle(
				url,
				titulo,
				ativo,
				ngeoxml);
		...
		adicionaNoArvoreGoogle : function(url, nomeOverlay, ativo, id) {
			...
			node = i3GEO.Interface.googlemaps.ARVORE.getNodeByProperty(
				"idkml",
				"raiz");
		...
		alteraParametroLayers : function(parametro, valor) {
			...
			var reg = new RegExp(
				parametro
					+ "([=])+([a-zA-Z0-9_]*)");
			i3GEO.Interface.googlemaps.PARAMETROSLAYER = i3GEO.Interface.googlemaps.PARAMETROSLAYER.replace(
				reg,
				"");
			i3GEO.Interface.googlemaps.PARAMETROSLAYER += "&"
				+ parametro
	...
	googleearth : {
		 ...
		 * (start code)
		 *
		 * i3GEO.Interface.googleearth.GADGETS = { setMouseNavigationEnabled :
		 * true, setStatusBarVisibility : true, setOverviewMapVisibility : true,
		 * setScaleLegendVisibility : true, setAtmosphereVisibility : true,
		 * setGridVisibility : false, getSun : false, LAYER_BORDERS : true,
		 * LAYER_BUILDINGS : false, LAYER_ROADS : false, LAYER_TERRAIN : true }
		 * (end)
		 */
		GADGETS : {
		...
		redesenha : function() {
			...
			for (i = 0; i < nlayers; i++) {
				...
				if (indice !== false) {
					try {
						i3GeoMap.getFeatures().removeChild(
							indice);
					} catch (e) {
					}
				}
		...
		iniciaGE : function(object) {
			var montaMapa = function(retorno) {
				i3GeoMap = object;
				i3GeoMap.getWindow().setVisibility(
					true);
				...
				if (i3GEO.parametros.kmlurl !== "") {
					i3GEO.Interface.googleearth.adicionaKml(
						true,
						i3GEO.parametros.kmlurl,
						i3GEO.parametros.kmlurl,
						false);
		...
		retornaIndiceLayer : function(nomeLayer) {
			...
			if (n > 0) {
				for (i = 0; i < n; i++) {
					if (i3GeoMap.getFeatures().getChildNodes().item(
						i).getName() === nomeLayer) {
						indice = i;
					}
				}
		...
		aplicaOpacidade : function(opacidade) {
			...
			for (i = 0; i < n; i++) {
				i3GeoMap.getFeatures().getChildNodes().item(
					i).setOpacity(
					opacidade);
			}
		},
		retornaObjetoLayer : function(nomeLayer) {
			...
			for (i = 0; i < n; i++) {
				if (i3GeoMap.getFeatures().getChildNodes().item(
					i).getName() === nomeLayer) {
					indice = i3GeoMap.getFeatures().getChildNodes().item(
						i);
				}
			}
			return indice;
		...
		balao : function(texto, ddx, ddy) {
			...
			b.setContentString("<div style=text-align:left >"
				+ texto
				+ "</div>");
		...
		adicionaKml : function(pan, url, titulo, ativo) {
			...
			eval(ngeoxml
				+ " = i3GeoMap.createNetworkLink('')");
			eval(ngeoxml
				+ ".setLink(linki3geokml)");
			if (i3GEO.arvoreDeCamadas.MOSTRALISTAKML === false) {
				i3GEO.arvoreDeCamadas.MOSTRALISTAKML = true;
				i3GEO.Interface.googleearth.criaArvoreKML();
			}
			i3GEO.Interface.googleearth.adicionaNoArvoreGoogle(
				url,
				titulo,
				ativo,
				ngeoxml);
		...
		ativaDesativaCamadaKml : function(obj) {
			var url = eval(obj.value
				+ ".getLink().getHref()"), existe = i3GEO.Interface.googleearth.existeLink(url);
			if (!obj.checked) {
				i3GEO.Interface.googleearth.ativaDesativaLink(
					url,
					false);
			} else {
				if (existe === false) {
					eval("i3GeoMap.getFeatures().appendChild("
						+ obj.value
						+ ")");
				} else {
					i3GEO.Interface.googleearth.ativaDesativaLink(
		...
		zoom2extent : function(mapexten) {
			var r = 6378700, lng2, lng1, lat1, lat2, ret = mapexten.split(" "), fov = 32, camera = i3GeoMap.getView().copyAsCamera(
			...
			i3GeoMap.getView().setAbstractView(
				camera);
		},
		alteraParametroLayers : function(parametro, valor) {
			parametro = parametro.toUpperCase();
			var reg = new RegExp(
				parametro
					+ "([=])+([a-zA-Z0-9_]*)");
			i3GEO.Interface.googleearth.PARAMETROSLAYER = i3GEO.Interface.googleearth.PARAMETROSLAYER.replace(
				reg,
				"");
			i3GEO.Interface.googleearth.PARAMETROSLAYER += "&"
				+ parametro
