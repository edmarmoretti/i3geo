 ...
 * impl&iacute;cita de COMERCIABILIDADE OU ADEQUACAtilde;O A UMA FINALIDADE
...
i3GEO.gadgets = {
	 ...
	 *
	 * Essa vari&aacute;vel define os parametros individuais de cada gadget e o
	 * ID do elemento HTML onde o gadget ser&aacute; inclu&iacute;do (parametro
	 * "idhtml").
	 *
	 * Cada tipo de gadget pode ter parametros espec&iacute;ficos, descritos a
	 * seguir.
	 *
	 ...
	 * (start code)
	 *
	 * i3GEO.gadgets.PARAMETROS = { "mostraInserirKml" : { idhtml : "inserirKml" },
	 * "mostraEscalaNumerica" : { idhtml : "escala" }, "mostraEscalaGrafica" : {
	 * idhtml : "escalaGrafica" }, "mostraBuscaRapida" : { idhtml :
	 * "buscaRapida", servicosexternos : true, temasmapa : false, google : true },
	 * "mostraVisual" : { idhtml : "" }, "mostraHistoricoZoom" : { idhtml :
	 * "historicozoom" }, "mostraMenuSuspenso" : { permiteLogin : true,
	 * marcadores : true, idhtml : "menus", deslocaEsquerda : 0, parametrosYUI : {
	 * iframe : false, autosubmenudisplay : false, showdelay : 200, hidedelay :
	 * 500, lazyload : false } }, "mostraMenuLista" : { idhtml : "menuLista" },
	 * "mostraVersao" : { idhtml : "versaoi3geo" }, "mostraEmail" : { idhtml :
	 * "emailInstituicao" } };
	 *
	 * (end)
	 */
	 ...
	 *
	 * Mostra o e-mail armazenado na vari&aacute;vel
	 * i3GEO.parametros.emailInstituicao
	 *
	 * Parametro:
	 *
	 * {String} - id do elemento HTML que receber&aacute; o resultado. Esse id
	 ...
	 *
	 * Mostra a vers&atilde;o atual do i3Geo armazenada na vari&aacute;vel
	 * i3GEO.parametros.mensageminicial
	 *
	 * Parametro:
	 *
	 * {String} - id do elemento HTML que receber&aacute; o resultado. Esse id
	...
	mostraVersao : function(id) {
		...
		i3GEO.util.defineValor(
			id,
			"innerHTML",
			i3GEO.parametros.mensageminicia);
	},
	/**
	 ...
	 *
	 * Se voc&ecirc; n&atilde;o quer essa fun&ccedil;&atilde;o no mapa, elimine
	 * o elemento HTML existente no mapa que contenha o id definido em
	 * i3GEO.gadgets.PARAMETROS.mostraInserirKml.idhtml
	 *
	 * Parametro:
	 *
	 * {String} - id do elemento HTML que receber&aacute; o resultado. Esse id
	 * por default &eacute; obtido de
	 * i3GEO.gadgets.PARAMETROS.mostraInserirKml.idhtml
	 */
	 ...
	 *
	 * A escala num&eacute;rica pode ser alterada pelo usu&aacute;rio
	 * digitando-se a nova escala.
	 *
	 * Se voc&ecirc; n&atilde;o quer essa fun&ccedil;&atilde;o no mapa, elimine
	 * o elemento HTML existente no mapa que contenha o id definido em
		...
		if (onde) {
			...
			if (!$i("i3geo_escalanum")) {
				i = "<form id='i3GEOescalanumForm' >"
					+ $inputText(
						...
						parseInt(
							...
							10))
					+ "</form>";
				ins = "<table style='width:120px;'><tr><td>"
					+ i;
				...
				$i("i3GEOescalanumForm").onsubmit = function() {
					i3GEO.navega.aplicaEscala(
						i3GEO.configura.locaplic,
						i3GEO.configura.sid,
						document.getElementById("i3geo_escalanum").value);
					return false;
				};
			}
	 ...
	 * Se voc&ecirc; n&atilde;o quer essa fun&ccedil;&atilde;o no mapa, elimine
	 * o elemento HTML existente no mapa que contenha o id definido em
	 * i3GEO.gadgets.PARAMETROS(escala)
	 *
	 * Parametro:
	 *
	 * {String} - id do elemento HTML que receber&aacute; o resultado. Esse id
	 ...
	 *
	 * Mostra a op&ccedil;&atilde;o de busca r&aacute;pida de lugares por
	 * palavra digitada.
	 *
	 * Se voc&ecirc; n&atilde;o quer essa fun&ccedil;&atilde;o no mapa, elimine
	 * o elemento HTML existente no mapa que contenha o id definido em
	 * i3GEO.gadgets.PARAMETROS (buscaRapida)
	 *
	 * Onde a busca ser&aacute; feita &eacute; controlado pela vari&aacute;vel
	 * i3GEO.gadgets.PARAMETROS.mostraBuscaRapida
	 *
	 * Parametro:
	 *
	 * {String} - id do elemento HTML que receber&aacute; o resultado. Esse id
	...
	mostraBuscaRapida : function(id) {
		...
		if ($i(id)) {
			...
			if (temp) {
				...
				temp.innerHTML = ins;
				$i("i3GEObotaoBuscaRapida"
					+ id).onclick = fbusca;
				$i("i3GEObotaoFormBuscaRapida"
					+ id).onsubmit = fbusca;
				$i("i3GEObotaoPropriedadesBuscaRapida"
					+ id).onclick = function() {
					...
					if (i3GEO.Interface.ATUAL === "googlemaps") {
						...
						ins += "<tr><td><input style=cursor:pointer onclick='i3GEO.gadgets.PARAMETROS.mostraBuscaRapida.google = this.checked' type=checkbox "
							+ google
							+ " ></td><td>Google</td></tr>";
					} else {
					...
					ins += "</table><br>"
						...
						+ $trad("x40")
						+ "</p>";
					$i("i3GEOpropriedadesBuscaRapida"
						+ id
						+ "_corpo").innerHTML = ins;
				};
			}
		}
	},
	 ...
	 *
	 * Mostra na barra de zoom os &iacute;cones que controlam a
	 * visualiza&ccedil;&atilde;o do hist&oacute;rico da navega&ccedil;&atilde;o
	 * sobre o mapa
	 *
	 * Parametro:
	 *
	 ...
	 *
	 * Gera os &iacute;cones e controla as op&ccedil;&otilde;es de
	 * modifica&ccedil;&atilde;o do visual do mapa.
	 *
	 * O visual consiste na defini&ccedil;&atilde;o dos &iacute;cones utilizados
	 * no mapa. O visual pode ser modificado na inicializa&ccedil;&atilde;o ou
	 * ent&atilde;o escolhido pelo usu&aacute;rio.
	 *
	 * Os visuais dispon&iacute;veis s&atilde;o definidos no servidor e
	 * consistem em diret&oacute;rios localizados em i3geo/imagens/visual. A
	 * lista de visuais dispon&iacute;veis &eacute; obtida na
	 * inicializa&ccedil;&atilde;o do i3geo.
	 *
	 * Os &iacute;cones para mudan&ccedil;a do visual s&atilde;o
	 * inclu&iacute;dos no elemento HTML definido em
	...
	visual : {
		 ...
		 * Parametro:
		 *
		 * id {String} - id do elemento que receber&aacute; os &iacute;cones
		 * (opcional)
		 */
		inicia : function(id) {
		...
		/**
		 * Troca o visual atual. A lista de visuais dispon&iacute;veis &eacute;
		 * obtida em i3GEO.parametros.listavisual
		 *
		 * Parametro:
		 *
	 ...
	 *
	 * Mostra o menu suspenso com op&ccedil;&otilde;es extras de an&aacute;lise,
	 * ajuda, etc
	 *
	 * Paradefinir os &iacute;cones existentes nos elementos principais do menu,
	 * edite o arquivo i3geo/css/botoes2.css e acrescente o estilo desejado.
	 * Utilize # para se referenciar ao elemento, cujo identificador &eacute;
	 * composto por "menu"+chave, exemplo #menuinterface ou #menuajuda
	 *
	 * O objeto YAHOO.widget.MenuBar resultante pode ser obtido na
	 * vari&aacute;vel i3GEOoMenuBar
	 *
	 ...
	 *
	 * Para executar uma opera&ccedil;&atilde;o ap&oacute;s o menu ser montado,
	 * utilize a propriedade
	 * i3GEO.gadgets.PARAMETROS.mostraMenuSuspenso.finaliza, por exemplo (a
	 * string &eacute; executada por meio da fun&ccedil;&atilde;o eval do
	 * javascript)
	 *
	 * Exemplo:
	 *
	 ...
	 * "");'; (end)
	 *
	 * O conte&uacute;do do menu &eacute; baseado na vari&aacute;vel
	...
	mostraMenuSuspenso : function(id) {
		...
		if (objid
			...
			if (objid) {
				...
				// HTML
				if (temp
					&& (!temp.style.height || parseInt(temp.style.height) === 0)) {
					temp.style.height = "30px";
				}
				if (objid.style.height
					&& parseInt(
						objid.style.height,
						10) === 0) {
				...
				ins += '<div class="bd" style="top:0px;'
					+ alinhamento
				...
				for (i = 0; i < n; i += 1) {
					...
					if (confm.submenus[confm.menu[i].id].length > 0) {
						ins += '<li class="yuimenubaritem" style="padding-top:2px;"><a style="'
							+ estilo
				...
				i3GEOoMenuBarLocal = new YAHOO.widget.MenuBar(
					id, i3GEO.gadgets.PARAMETROS.mostraMenuSuspenso.parametrosYUI);
		...
		try {
			...
			if (temp != ""
				&& $i(temp)) {
				i3GEOoMenuBar.getMenuItem(temp).cfg.setProperty(
					"checked",
					true);
			}
		} catch (e) {
	 ...
	 *
	 * Mostra as op&ccedil;&otilde;es existentes no menu suspenso por&eacute;m
	 * na forma de uma lista de op&ccedil;&otilde;es
	 *
	 * O conte&uacute;do do menu &eacute; baseado na vari&aacute;vel
	 * i3GEO.configura.oMenuData
	 *
	...
	mostraMenuLista : function(id) {
		var objid, n, i, sub, nomeMenu = "", ms = i3GEO.gadgets.PARAMETROS.mostraMenuLista, confm = i3GEO.configura.oMenuData, ins = "", subs = i3GEO.configura.oMenuData.submenus;
		if (arguments.length === 0) {
			id = ms.idhtml;
		} else {
			ms.idhtml = id;
		...
		if (objid) {
			n = confm.menu.length;
			for (i = 0; i < n; i += 1) {
				ins += '<div class="listaMenuTitulo" id=menulista_'
					+ confm.menu[i].id
					+ '>'
					+ confm.menu[i].nome
					+ '</div>';
			...
			for (nomeMenu in subs) {
				if ($i("menulista_"
					...
					ins = "";
					for (i = 0; i < n; i++) {
						ins += "<p class='listaMenuItem' ><a href='"
							+ sub[i].url
							+ "' target='_self'>"
							+ sub[i].text
							+ "</a>";
