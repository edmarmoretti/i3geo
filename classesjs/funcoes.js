...
function initJanelaMen()
{
	if (!$i("janelaMen"))
	{
		...
		var temp = '<div class="hd" style="background-color:rgb(230,230,230)"><div class="tl"></div>';
		temp += '<div class="tr"></div></div>';
		temp += '<div id=janelaMenTexto style="color:rgb(170,170,170)">'+g_mensagempadrao+'</div>';
		...
		YAHOO.janelaMen.xp.panel = new YAHOO.widget.Panel("janelaMen", { width:"266px", height:"100px", fixedcenter: false, constraintoviewport: true, underlay:"none", close:true, visible:true, draggable:true, modal:false } );
...
function docaguias()
{
	if (!$i("conteudojanelaguias"))
	{
		...
		novoel.style.display="block";
		var temp = '<div class="hd"><div class="tl"></div>';
		temp += '<div class="tr"></div></div>';
		temp += '<div id=conteudojanelaguias ></div>';
		novoel.innerHTML = temp;
		document.body.appendChild(novoel);
...
function ativaGuias()
{
	...
	for (i=0;i<10;i++)
	{
		...
		{
			document.getElementById("guia"+i).style.borderBottom="1px solid rgb(210,210,210)";
			document.getElementById("guia"+i).onmousedown = function()
			{
				for (j=0;j<10;j++)
				{
					...
					{
						//document.getElementById("guia"+j).style.backgroundColor="rgb(250,250,250)";
...
function wdocaf(wlargura,waltura,wsrc,nx,ny,texto)
{
	...
	if (!$i("wdoca"))
	{
		...
		var ins = '<div class="hd"><div class="tl"></div>';
		ins += '<div class="tr"></div></div><div class="bd" style="scrollbar-arrow-color:white;scrollbar-base-color:white;scrollbar-darkshadow-color:white;scrollbar-face-color:white;scrollbar-highlight-color:white;scrollbar-shadow-color:white;" >';
		ins += '<iframe name=wdocai id=wdocai valign="top" style="border:0px white solid"></iframe></div>';
		novoel.innerHTML = ins;
		document.body.appendChild(novoel);
	}
	$i("wdocai").src = "";
	YAHOO.janelaDoca.xp.panel = new YAHOO.widget.ResizePanel("wdoca", { width:wlargura_, fixedcenter: false, constraintoviewport: false, underlay:"none", close:true, visible:true, draggable:true, modal:false } );
	YAHOO.janelaDoca.xp.panel.moveTo(imagemxi,imagemyi+50);
	YAHOO.janelaDoca.xp.panel.render();
	YAHOO.janelaDoca.xp.panel.show();
	...
	if(navn)
	{with ($i("wdocai").style){width = "100%";height = parseInt(waltura)-20+"px";overflow="-moz-scrollbars-vertical;overflow-x:scroll"};}
...
function wdocaf2(wlargura,waltura,wsrc,nx,ny,texto)
{
	if (!$i("wdoca2"))
	{
		...
		novoel.style.display="none";
		var ins = '<div class="hd"><div class="tl"></div>';
		ins += '<div class="tr"></div></div><div class="bd">';
		ins += '<iframe name=wdocai2 id=wdocai2  valign="top" ></iframe></div></div>';
		novoel.innerHTML = ins;
		document.body.appendChild(novoel);
	}
	YAHOO.namespace("janelaDoca2.xp");
	YAHOO.janelaDoca2.xp.panel = new YAHOO.widget.Panel("wdoca2", {width:wlargura, fixedcenter: false, constraintoviewport: true, underlay:"matte", close:true, visible:true, draggable:true, modal:true } );
	YAHOO.janelaDoca2.xp.panel.moveTo(imagemxi,imagemyi);
	YAHOO.janelaDoca2.xp.panel.render();
	YAHOO.janelaDoca2.xp.panel.show();
	with ($i("wdocai2").style){width = wlargura;height = waltura;}
...
hlpt - arquivo de help que deverá ser aberto se a tecla "a" for pressionada. O arquivo é passado para
a variável global g_hlpt.
*/
...
function mostraguiaf(guia)
{
	if ($i("guia"+guia))
	{
		...
		for (j=0;j<fs.length; j++)
		{
			...
			{
				jj = fs[j];
				$i("guia"+jj).style.backgroundColor="rgb(240,240,240)";
				if ($i("guia"+jj+"obj"))
				{$i("guia"+jj+"obj").style.display="none";}
			}
		}
		$i("guia"+guia).style.backgroundColor="rgb(210,210,210)";
		if ($i("guia"+guia+"obj"))
...
function borra(tipo)
{
	...
	if (!$i("borraID"))
	{
		...
		novoel.id = "borraID";
		novoel.style.display="none";
		novoel.style.backgroundColor="white";
		if (navm)
		{
		...
		with($i("borraID").style){width = w;position="absolute";top=0}
		with($i("borraID").style){height = h;position="absolute";left=0}
	}
	if (tipo == "sim")
	{$i("borraID").style.display="block";}
	else
	{$i("borraID").style.display="none";}
	*/
}
/*
Function: aguarde
...
function aguarde()
{
	...
	//fecha o aguarde da janela docavel se existir
}
/*
...
function ativaClicks(docMapa)
{
			docMapa.onmouseover = function()
			{
				...
				this.onmousemove=function(exy)
				{
					if ($i("tip"))
					{$i("tip").style.display="none";}
					capturaposicao(exy);  // captura a posiÃ¯Â¿Â½o do mouse
					if (g_destaca != "")
					...
					movecursor();  //move o icone do mouse
					if ($i("longlat"))
					...
					if (g_tipoacao == "mede")
					{
						...
						if (n > 0) //com dois pontos da para calcular a distancia
						{
			...
			docMapa.onmousedown = function()
			{
				...
				{
					g_panM = "sim";
					leftinicial = parseInt($i("corpoMapa").style.left);  //imagemxi // marca a localizacao inicial do corpo do mapa
					topinicial = parseInt($i("corpoMapa").style.top); //imagemyi
					clicinicialx = objposicaocursor.imgx; // pega o clic inicial
...
function initJanelaZoom(qual)
{
	...
	if ((qual == 1) && (!$i("maisBotoes1")))
	{
		...
		if ($i("zoomli"))
		{
			...
			temp += '<div id=vertMenosZoom onmouseover="mostradicasf(this,\'Reduz o mapa mantendo o centro atual.\',\'\')" onclick="zoomoauto()"  ></div></div></div>';
		}
		temp += '<div id=maisBotoes1 ></div>';
		novoel.innerHTML = temp;
		document.body.appendChild(novoel);
		//copia os botoes do HTML para a janela
		if ($i("barraDeBotoes1"))
		{
			$i("maisBotoes1").innerHTML = $i("barraDeBotoes1").innerHTML;
			$i("barraDeBotoes1").innerHTML = "";
		}
		YAHOO.namespace("janelaBotoes1.xp");
		YAHOO.janelaBotoes1.xp.panel = new YAHOO.widget.Panel("janelaBotoes1", {width:"36px", fixedcenter: false, constraintoviewport: false, underlay:"none", close:true, visible:true, draggable:true, modal:false } );
	...
	if ((qual == 2) && (!$i("maisBotoes2")))
	{
		...
		YAHOO.janelaBotoes2.xp.panel = new YAHOO.widget.Panel("janelaBotoes2", {width:"36px", fixedcenter: false, constraintoviewport: false, underlay:"none", close:true, visible:true, draggable:true, modal:false } );
...
function initJanelaRef()
{
	if (!$i("winRef"))
	{
		...
		novoel.style.display="none";
		var ins = '<div class="hd"><div class="tl"></div>';
		ins += '<div class="tr"></div></div><div class="bd" id="mapaReferencia" onmouseover="javascript:movimentoRef(this)" onclick="javascript:clicouRef()">';
		ins += '<img style="cursor:pointer" id=imagemReferencia src="" /></div></div>';
		ins += '<div style="text-align:left;font-size:0px" id=refmensagem ></div>';
		novoel.innerHTML = ins;
		novoel.style.borderColor="gray";
		document.body.appendChild(novoel);
	}
	...
	YAHOO.janelaRef.xp.panel = new YAHOO.widget.Panel("winRef", { width:"160px", fixedcenter: false, constraintoviewport: true, underlay:"none", close:true, visible:true, draggable:true, modal:false } );
	YAHOO.janelaRef.xp.panel.render();
	if (navm){YAHOO.janelaRef.xp.panel.moveTo((imagemxi+objmapa.w-160),imagemyi+4);}
	else
	{YAHOO.janelaRef.xp.panel.moveTo((imagemxi+objmapa.w-167),imagemyi+4);}
...
function mudaboxnf(tipo)
{
	...
	if ($i("aplicari"))
	{
		...
		if (navn)
		{
			with ($i("aplicari").style)
			{
				left = objposicaomouse.x;
				top = objposicaomouse.y+document.body.scrollTop;
			}
		}
	}
}
...
function movelentef()
{
	if ($i("lente"))
	{
		if ($i("lente").style.visibility=="visible") // movimenta?o da lente
		{
...
function verificaTip()
{
	...
	//var anim = new YAHOO.util.Anim('tip', { opacity: { from: 0, to: 0.5 } }, 1, YAHOO.util.Easing.easeIn);
	if ((objmapa.parado == "parar") || (objmapa.parado=="cancela")){return;}
	//if (navm)
	//{$i("tip").style.filter = "alpha(opacity=90)";}
	//else
	//{$i("tip").style.opacity=".9";}
	...
	if (!document.getElementById("idx"+itemID))
	{
		for (l=0;l<lista.length; l++)
		{
			...
			if (ltema[0] == itemID)
			{
				...
				{mytreeview1.createItem("opc"+ltema[0], "Opções", imgBranco, true, true, true, ltema[0]);}
...
function remapaf()
{
	...
	if ($i(objmapa.guiaTemas+"obj"))
	{
		...
		var tsd = new Array(); //temas desligados
		...
		var remapaAdicNovos = function remapaAdicNovos(retorno)
		{
			if ($i("buscatema"))
			{
				...
				var ta = new Array(); //temas ligados
				for (i=0;i<iguias.length; i++)
				{
					if (iguias[i].type == "checkbox")
					{
...
function calcposf()
{
	imagemxi = 0;  //posicao da tag img
	imagemyi = 0;
	imagemxref = 0;  //posicao da tag ref
	imagemyref = 0;
	if(!$i("corpo")){return;}
	var dc = $i("corpo");
	if ($i("img"))
	...
	{
		var dc = $i("ref");
		while (dc.offsetParent.id != "corpo")
		{
			dc = dc.offsetParent;
...
function capturaposicao(exy)
{
	...
	if (navn)
	{
		...
		calcposf(); // calcula a posiï¿½o correta do mouse sobre o tag [img] e [ref] calcula imagemxi...
...
function gravaQuadro(variavel,valor)
{
	...
	if ($i("lugarquadros"))
	{
		...
		{
			//if ((quadrosfilme[i - 2]) && (quadrosfilme[i - 2].imagem == " "))
			//{rebobinaf();return;}	
...
function filmeanimaf()
{
	...
	for (i = 0; i < quadrosfilme.length; i++)
	{
		...
		{
			preLoad[i] = new Image();  //cria array para armazenar as imagens
			preLoad[i].src = quadrosfilme[i].imagem;//carrega as imagens no array
		}
	}
	filmeanimarodaf(0);
}
...
function calculadistancia(lga,lta,lgb,ltb) //0ms
{
	...
	return theta*er; //retorna a distancia em km
}
/*
Function: convdmsddf
...
function calcddf(xfign,yfign,g_celula,imgext)
{
	...
	if (navn)
	{
	 xfign = xfign - 0.12; //2.2 098 3 0136
	 yfign = yfign - 1.05;
	}
...
*/
function convddtela(vx,vy,docmapa) //0ms
{
	if(!docmapa)
	...
	xy = new Array();
	xy[0] = (vx  / c) + imagemxi;  //x
	xy[1]  = (vy / c) + imagemyi;  //y
	return (xy);
}
/*
...
function posicaocursor()
{
	...
	this.imgx = 0;
	this.imgy = 0;
	this.refx = 0; //posicao sobre a tag ref
	this.refy = 0; //posicao sobre a tag ref
}
/*
Function: pontosdist
