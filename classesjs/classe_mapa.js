...
i3GEO.mapa =
	{
		 ...
		 *
		 * Lista indicando quais temas serao adicionados ao mapa na
		 * inicializacao
		 *
		 * Esse valor e utilizado para completar a variavel mashuppar utilizada
		 * por i3GEO.
		 *
		 ...
		 *
		 * Indica se o tamanho do mapa sera ajustado toda vez que o navegador
		 * for redimensionado
		 *
		 * Tipo:
		 *
		 ...
		 *
		 * Resolu&ccedil;&atilde;o de busca utilizada no bal&atilde;o de
		 ...
		 *
		 * Insere o icone do tipo "dobra de pagina" que permite alterar o
		...
		insereDobraPagina : function(tipo, imagem) {
			...
			novoel.style.top = i3GEO.parametros.h
				- 35
				+ "px";
			novoel.style.zIndex = "5000000";
			novoel.style.left = i3GEO.parametros.w
				- 35
				+ "px";
			$i(
		...
		/**
		 * Ativa o redimensionamento automatico do mapa sempre que o navegador
		 * for redimensionado
		 *
		...
		ativaAutoResize : function() {
			...
			window.onresize = function() {
				...
				if (Math.abs(Dw
					- i3GEO.tamanhodoc[0]) > 50) {
					r = true;
				}
				if (Math.abs(Dh
					- i3GEO.tamanhodoc[1]) > 50) {
					r = true;
				}
				if (r === false) {
					// return;
		 ...
		 *
		 * Esse ajuste e necessario na inicializacao, uma vez que o mapa utiliza
		 * style.position='absolute'
		 *
		 * Parameters:
		 *
		 * elemento {String} - id do elemento HTML que devera ser ajustado e que
		 * contem o mapa
		 */
		ajustaPosicao : function(elemento) {
			...
			try {
				...
				i3GEO.janela.tempoMsg("Ocorreu um erro. i3GEO.mapa.ajustaPosicao "
					+ e);
			}
		},
		/**
		 * Function: ativaTema
		 *
		 * Altera a variavel i3GEO.temaAtivo e atualiza a interface em funcao do
		 * novo tema que for ativado
		 *
		 * O tema anteriormente ativo tem sua cor alterada para a cor normal e o
		 * novo tema e destacado com uma cor diferente
		 *
		...
		/**
		 * Verifica se ocorreu algum problema na atualizacao do corpo do mapa e
		 * inicia o processo de tentativa de recuperacao
		 *
		 * Parametro:
		 *
		 ...
		 *
		 * O i3Geo mantem sempre uma copia do arquivo mapfile em uso. Essa
		 * funcao tenta usar essa copia para restaurar o funcionamento do mapa
		 */
		recupera : {
			/**
			 * Armazena a quantidade de tentativas de recuperacao que foram
			 * feitas
			 *
			 * Tipo {Integer}
			 */
		...
		legendaIMAGEM : {
			 ...
			 * Parametro:
			 *
			 * funcao {function} - funcao que recebera o resultado da chamada
			 * AJAX. O objeto CPAINT e enviado como parametro.
			 */
		 ...
		 *
		 * Retorna uma string no formato base64 + JSON contendo as geometrias
		 * existentes no layer grafico do mapa Essas geometrias podem ser
		...
		compactaLayerGrafico : function() {
			...
			if (i3GEO.editorOL
				...
				&& i3GEO.desenho.layergrafico.features) {
		...
		desCompactaLayerGrafico : function(geometrias) {
			...
			if (geometrias.length > 0) {
				var inicia = function() {
					...
					for (i = 0; i < n; i++) {
						i3GEO.editorOL.adicionaFeatureWkt(
							geometrias[i].geometria,
							geometrias[i].atributos);
					}
					...
					try {
						...
						if (temp[0] == "i3GEO.configura.mapaRefDisplay") {
							i3GEO.util.insereCookie(
								"i3GEO.configura.mapaRefDisplay",
								temp[1]);
						}
					} catch (e) {
					}
		...
		legendaHTML : {
			 ...
			 *
			 * Define se na legenda sera incluido o botao para liberar a legenda
			 * e inclui-la em uma janela flutuante
			 *
			 * Tipo:
			 *
			 * {boolean}
			 ...
			 *
			 * A legenda e incluida no id definido. Se id for igual a "", sera
			 * apenas definido o evento de atualizacao, permitindo que seja
				...
				var idleg = $i("wlegenda_corpo"), temp =
					function(retorno) {
						...
						if (retorno.data !== "erro"
							...
							re = new RegExp(
								"legendack_", "g");
							retorno.data.legenda = retorno.data.legenda.replace(
								re,
								"liblegendack_");
							legenda =
						...
						if (legenda != ""
							...
							legenda = legenda.replace(
								re,
								"");
							ins += "<div id='corpoLegi' >"
								+ legenda
								+ "</div>";

							idleg.innerHTML = "<div style='padding:5px;' >"
								+ ins
								+ "</div>";
						}
			 ...
			 * O resultado e processado pela funcao passada como parametro
			 *
			 * O template utilizado para gerar a legenda &eacute; definido em
			 * i3GEO.configura.templateLegenda
			 *
			 * Parametro:
			 *
			 * funcao {function} - funcao que recebera o resultado da chamada
			...
			/**
			 * Liga ou desliga um unico tema. Utilizado pela legenda HTML,
			 * permitindo que um tema seja processado diretamente na legenda.
			 *
			 * Parametro:
			 *
			 * inputbox {object) - objeto do tipo input checkbox com a
			 * propriedade value indicando o codigo do tema que sera processado
			 */
			 ...
			 * Parametros:
			 *
			 * {sim|nao} - (opcional) inclui ou nao o checkbox que permitem
			 * desligar a camada
			 *
			...
			libera : function(ck, largura, altura, topo, esquerda) {
				...
				if (!$i("wlegenda")) {
					...
					titulo = "<span class='i3GEOconeFerramenta i3GEOiconeLegenda' title='"
						+ $trad("P3")
						+ "'></span>"
		...
		dialogo : {
			...
			wkt2layer : function(wkt, texto) {
				var temp = function() {
					i3GEOF.wkt2layer.iniciaJanelaFlutuante(
						wkt,
						texto);
				};
				i3GEO.util.dialogoFerramenta(
					"i3GEO.mapa.dialogo.wkt2layer()",
					"wkt2layer",
					"wkt2layer",
					"dependencias.php",
					temp);
			 ...
			 *
			 * Janela com as principais opcoes de edicao de um layer. Utilizado
			 * pelo sistema de administracao
			 *
			 * Parametro:
			 *
			 ...
			 * Function: geolocal
			 *
			 * Abre a janela de dialogo da ferramenta de geolocalizacao do
			 * usuario
			 */
			geolocal : function() {
				i3GEO.util.dialogoFerramenta(
			...
			listaDeMapasBanco : function() {
				if (i3GEO.guias.CONFIGURA["mapas"]) {
					...
					// como uma guia
					i3GEO.guias.CONFIGURA["mapas"].click.call(
						this,
						divid);
				} else {
					window.open(
			 ...
			 *
			 * Congela a vis&atilde;o atual do mapa mostrando-o em uma janela
			 * flutuante
			 */
			congelaMapa : function() {
				...
				if (i3GEO.Interface.ATUAL === "openlayers"
					...
					url = i3GEO.configura.locaplic
						+ "/ferramentas/congelamapa/openlayers.php?g_sid="
						+ i3GEO.configura.sid
						+ "&ext="
						+ i3GEO.util.extOSM2Geo(i3GEO.parametros.mapexten);
					titulo = "<span class='i3GEOconeFerramenta i3GEOiconeCongela'></span>"
						+ "Mapa"
			 ...
			 *
			 * Abre a janela de dialogo da ferramenta de busca de regioes
			 * cadastradas no METAESTAT com opcao de filtrar a regiao escolhida
			 */
			 ...
			 * Function: animacao
			 *
			 * Abre a janela de dialogo da ferramenta que permite animar o mapa
			 * atual
			 */
			animacao : function() {
				i3GEO.util.dialogoFerramenta(
			 ...
			 * Function: opacidade
			 *
			 * Abre a janela de dialogo da ferramenta de definicao da
			 * transparencia das camadas principais do mapa
			 */
			opacidade : function() {
				i3GEO.util.dialogoFerramenta(
			 ...
			 * Function: telaRemota
			 *
			 * Abre a janela de dialogo da ferramenta de configuracao de uma
			 * tela remota
			 */
			telaRemota : function() {
				i3GEO.util.dialogoFerramenta(
					"i3GEO.mapa.dialogo.telaremota()",
					"telaremota",
					"telaremota");
			},
			/**
			 * Function: t3d
			 *
			 * Abre a janela de dialogo da ferramenta de geracao da visualizacao
			 * em 3d
			 */
			t3d : function() {
				i3GEO.util.dialogoFerramenta(
			 ...
			 * Function: imprimir
			 *
			 * Abre a janela de dialogo da ferramenta que permite imprimir o
			 * mapa atual
			 */
			imprimir : function() {
				i3GEO.util.dialogoFerramenta(
			 ...
			 *
			 * Abre a janela de dialogo da ferramenta que mostra a extensao
			 * geografica atual do mapa
			 */
			...
			salvaMapa : function() {
				if (i3GEO.parametros === "") {
					i3GEO.janela.tempoMsg("Essa opcao nao pode ser ativada. Consulte o administrador do sistema. Mapfile nao esta exposto.");
					return;
				}
				i3GEO.util.dialogoFerramenta(
			...
			template : function() {
				i3GEO.janela.cria(
					...
					"",
					"",
					"Template <a class=ajuda_usuario target=_blank href='"
						+ i3GEO.configura.locaplic
			...
			cliqueIdentificaDefault : function(x, y) {
				...
				if (i3GEO.barraDeBotoes.BOTAOCLICADO !== "identifica"
				...
				if (typeof (i3GEOF.identifica) === 'undefined') {
					...
					var js = i3GEO.configura.locaplic
						+ "/ferramentas/identifica/dependencias.php", temp = function() {
						if (x) {
			...
			/**
			 * Mostra etiquetas no mapa com informacoes sobre os temas com
			 * etiquetas ativas
			 *
			...
			verificaTipDefault : function(e) {
				...
				if (i3GEO.barraDeBotoes.BOTAOCLICADO !== "identificaBalao"
					&& i3GEO.eventos.cliquePerm.ativo === false) {
					return;
				}
				// na interface googleearth verifica se existe mais eventos no
				...
				retorna = function(retorno) {
					...
					if (retorno !== "") {
						...
						for (j = 0; j < ntemas; j += 1) {
							titulo = temas[j].nome;
							if (tipotip != "simples") {
								titulo = "<span class='toolTipBalaoTitulo'><b>"
									+ titulo
									+ "</b></span><br>";
							} else {
								titulo = "";
