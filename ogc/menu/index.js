...
Obt&eacute;m a lista de menus
    ...
    lista: function () {
        $.post(
            "exec.php",
            "funcao=lista&id_menu=" + ogc.menu.id_menu
        )
            .done(
                function (data, status) {
                    ...
                    var htmlpastas = Mustache.to_html(
                        ...
                        $.extend(
                            ...
                            {
                                "data": json["dados"],
                                "onclick": "ogc.menu.proximoNivel",
                            }
                        )
                    );
                    ogc.menu.ondeLista.html(htmlcamadas + htmlpastas);
                    $.material.init();
                }
            )
            .fail(function (data) {
                ogc.menu.ondeLista.html('<div class="alert alert-danger alert-dismissible" role="alert">' + data.status + " " + data.statusText + '</div>');
            });
    },
    ...
    mostraLinksDownload: function (codigo_tema) {
        ...
        $.get(
            ...
            .done(
                function (data, status) {
                    var retorno = JSON.parse(data).data;
    ...
    proximoNivel: function (id, nome) {
        window.location.href = "grupo/index.php?id_grupo=" + id + "&nome_grupo=" + nome + "&id_menu=" + ogc.menu.id_menu + "&nome_menu=" + ogc.menu.nome_menu;
    }
};
