...
i3GEOadmin.bdExplorer = {
		...
		listaEsquemasUpload: function(destino){
			if($(".popover").length == 0){
				$.post(
						i3GEO.configura.locaplic + "/admin1/php/bdexplorer.php?funcao=listarEsquemasUpload"
				)
				.done(
						function(data, status){
							var c = "", json = jQuery.parseJSON(data);
		...
		listaTabelas: function(codigo_estat_conexao,esquema,destino){
			if($(".popover").length == 0){
				$.post(
						i3GEO.configura.locaplic + "/admin1/php/bdexplorer.php?funcao=listarTabelas",
		...
		listaTabelasUpload: function(esquema,destino){
			if($(".popover").length == 0){
				$.post(
						i3GEO.configura.locaplic + "/admin1/php/bdexplorer.php?funcao=listarTabelasUpload",
						"esquema=" + esquema
				)
				.done(
						function(data, status){
		...
		listaColunas: function(codigo_estat_conexao,esquema,tabela,destino){
			if($(".popover").length == 0){
				$.post(
						i3GEO.configura.locaplic + "/admin1/php/bdexplorer.php?funcao=listarColunas",
		...
		listaCodigosConexao: function(destino){
			if($(".popover").length == 0){
				$.post(
						i3GEO.configura.locaplic + "/admin1/php/bdexplorer.php?funcao=listarCodigosConexao"
				)
				.done(
						function(data, status){
							var c = "", json = jQuery.parseJSON(data);
