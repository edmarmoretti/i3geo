...
\admin\php\funcoesAdmin\testaSafeNumerico(array($codigo_variavel));

$funcao = strtoupper ( $funcao );
switch ($funcao) {
	case "ADICIONAR" :
		$novo = \admin\metaestat\variaveis\adicionar ( $_POST["nome"], $_POST["descricao"], $dbhw );
		$dbhw = null;
		$dbh = null;
		if ($novo === false) {
		...
		break;
	case "ALTERAR" :
		$novo = \admin\metaestat\variaveis\alterar ( $codigo_variavel,$_POST["nome"], $_POST["descricao"], $dbhw );
		$dbhw = null;
		$dbh = null;
		if ($novo === false) {
			header ( "HTTP/1.1 500 erro ao consultar banco de dados" );
	...
	case "LISTAUNICO" :
		...
		break;
	case "LISTA" :
		$dados = \admin\metaestat\variaveis\listar ( $dbh );
		$dbhw = null;
		$dbh = null;
		if ($dados === false) {
			header ( "HTTP/1.1 500 erro ao consultar banco de dados" );
		...
		break;
	case "EXCLUIR" :
		$retorna = \admin\metaestat\variaveis\excluir ( $codigo_variavel, $dbhw );
		$dbhw = null;
		$dbh = null;
		if ($retorna === false) {
			header ( "HTTP/1.1 500 erro ao consultar banco de dados" );
