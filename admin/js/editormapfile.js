...
function montaNosRaiz(redesenha)
{
	...
	for (var i=0, j=$mapfiles.length; i<j; i++)
	{
		...
		conteudo += "&nbsp;<img width=20px style=\"position:relative;cursor:pointer;top:2px\" onclick=\"javascript:window.open('../../ms_criamapa.php?layers="+$mapfiles[i].codigo+"."+$mapfiles[i].extensao+"')\" title='abrir no i3Geo' src=\"../../imagens/i3geo2.jpg\" />";
...
function montaEditorDados(dados)
{
	...
	if(dados.postgis_mapa.length > 0)
	{
		...
		ins += "<br>Os campos em cores n&atilde;o s&atilde;o compat&iacute;veis com o tipo de conex&atilde;o.</p><br>";
	}
