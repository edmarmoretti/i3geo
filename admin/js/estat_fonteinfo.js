function initEditor(){
	...
	core_ativaBotaoAdicionaLinha("../php/metaestat.php?funcao=alterarFonteinfo","adicionaNovaLinha","pegaDados");
	pegaDados();
}
function pegaDados(){
	core_carregando("ativa");
	core_pegaDados("buscando dados...","../php/metaestat.php?funcao=listaFonteinfo","montaTabela");
}
function montaTabela(dados){
	YAHOO.example.InlineCellEditing = new function()	{
		...
		myDataSource.responseSchema = {
				fields: ["id_fonteinfo","titulo","link"]
		};
		myDataTable.subscribe(
			...
			function(ev){
				...
				if (column.key == 'excluir'){
					...
					excluiLinha(record.getData('id_fonteinfo'),target);
				}
				if (column.key == 'mais'){
					record = this.getRecord(target);
					core_carregando("ativa");
					core_carregando("buscando dados...");
					$clicouId = record.getData('id_fonteinfo');
					$recordid = record.getId();
					sUrl = "../php/metaestat.php?funcao=listaFonteinfo&id_fonteinfo="+record.getData('id_fonteinfo');
					callback = {
							success:function(o){
								try{
...
function gravaDados(id,recordid){
	var campos = new Array("titulo","link"),
		par = "",
		i = 0,
		sUrl,callback;
