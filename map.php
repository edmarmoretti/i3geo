...
for ($i = 0; $i < $numlayers; ++ $i) {
    ...
    if ($layerName == $_GET["layer"] && ($_GET["FORMAT"] == "utfgrid")) {
        $_GET["SRS"] = "EPSG:3857";
        include ("classesphp/mapa_utfgrid.php");
        exit();
    }
}
//
...
function salvaCacheImagem()
{
    ...
    $c = $cachedir . "/wmts/$layer/$z/$x";
    if (! file_exists($c . "/$y.png")) {
        if (! file_exists($cachedir . "/wmts/$layer/$z/$x")) {
            mkdir($cachedir . "/wmts/$layer/$z/$x", 0744, true);
            chmod($cachedir . "/wmts/$layer/$z/$x", 0744);
        }
