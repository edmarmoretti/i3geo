...
i3GEO.coordenadas =
{
	...
	mostraCoordenadasJanela: function (x, y) {
		...
		for (i = 0; i < n; i += 1) {
			...
			if (temp.ativo === true) {
				if (temp.tipo === "geo") {
					ins += i3GEO.coordenadas.criaMascaraDMS("coordgeotabela" + tipos[i], temp.titulo, temp.titulo);
				} else {
					if (temp.tipo === "codigo") {
						ins += i3GEO.coordenadas.criaMascaraCodigo("coordgeotabela" + tipos[i], temp.titulo, temp.titulo, temp.tipoCodigo);
					} else {
						ins += i3GEO.coordenadas.criaMascaraMetrica("coordgeotabela" + tipos[i], temp.titulo, temp.titulo);
					}
				}
			}
		}
		...
		let close = function () {
			//remove os eventos
			i3GEO.coordenadas.captAltC.remove();
		...
		ins += "<h6><a href='javascript:void(0)' style='cursor:pointer;' onclick='i3GEO.coordenadas.captAltC.ativa();return false;'>" +
			"Clique aqui para ativar o Alt+c para poder capturar as coordenadas em mem&oacute;ria</a></h6>";


		i3GEO.janela.formModal(
			{
				texto: "<div style='text-align:left;margin-left:10px;'>" + ins + "</div>",
				resizable: {
					disabled: false,
					ghost: true,
					handles: "se,n"
				},
				css: { 'cursor': 'pointer', 'position': 'fixed', 'top': 0, 'left': 0, 'right': 0, bottom: 'unset', 'margin': 'auto' },
				onclose: close
			});
	...
	captAltC: {
		...
		ativa: function () {
			if (i3GEO.coordenadas.captAltC.evento == false) {
				document.addEventListener("keydown", i3GEO.coordenadas.captAltC.captura);
				i3GEO.coordenadas.captAltC.evento = true;
			}
		},
		remove: function () {
			if (i3GEO.coordenadas.captAltC.evento == true) {
				document.removeEventListener("keydown", i3GEO.coordenadas.captAltC.captura);
				i3GEO.coordenadas.captAltC.evento = false;
			}
		}
	},
	 ...
	 * Veja:
	 *
	 * http://localhost/i3geo/pacotes/geohash
	 */
	geohash: {
		BITS: [
			16, 8, 4, 2, 1
