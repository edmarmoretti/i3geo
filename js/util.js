...
i3GEO.util =
{
	 ...
	 * Parametros:
	 *
	 * {String} - (depreciado) id do conteudo da janela flutuante que chamou a funcao. Pode ser "" caso elemento exista em document
	 *
	...
	abreCor: function (janelaid, elemento, tipo) {
		let el = document.getElementById("i3GEOcolorPicker");
		if (!el) {
			el = document.createElement("div");
			document.body.insertBefore(el, $i(i3GEO.Interface.IDCORPO));
			el.id = "i3GEOcolorPicker";
		...
		const pickr = new Pickr({
			...
			appClass: 'i3geo-colorpicker-class',
		...
		pickr.on('save', (color, instance) => {
			instance.setColorRepresentation("RGBA");
			let cor = instance.getSelectedColor().toRGBA();
			//console.log(cor);
			let sep = ",";
			if (tipo == "rgbSep") {
				sep = " ";
			}
			ncor = parseInt(cor[0], 10) + sep + parseInt(cor[1], 10) + sep + parseInt(cor[2], 10);
			instance.hide(); // Hide modal
			document.getElementById(elemento).value = ncor;
			try {
				document.getElementById(elemento).onchange.call();
			}
			catch (e) { }
		}).on('cancel', instance => {
			instance.hide(); // Hide modal
	...
	scriptTag: function (js, ini, id, aguarde) {
		console.log("i3GEO.util.scriptTag ");
		console.log("========= " + id);
	...
	dialogoFerramenta: function (mensagem, dir, nome, nomejs, nomefuncao) {
		console.log("i3GEO.util.dialogoFerramenta " + dir);
		if (i3GEO.parametros.i3geoBlFerramentas.lastIndexOf(dir) >= 0) {
			console.warn(dir + " ================== Bloqueado ==================");
			return;
		}
