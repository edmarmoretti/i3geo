...
i3GEO.editorOL =
	{
		...
		criaComboTemas : function() {
			var layers = i3GEO.editorOL.layersLigados(), nlayers = layers.length, i, nometema = "", temp, combo =
		...
		atualizaJanelaAtivaTema : function() {
			...
			document.getElementById("i3GEOOLlistaTemasAtivos").onchange = function() {
				if (botaoIdentifica) {
					botaoIdentifica.layers = [
						i3GEO.editorOL.layersLigados()[this.value]
					];
				}
			};
		},
		...
		criaBotoes : function(botoes) {
			...
			if (botoes.texto === true) {
				button = new OpenLayers.Control.DrawFeature(i3GEO.desenho.layergrafico, OpenLayers.Handler.Point, {
					...
					callbacks : {
							...
							var texto = window.prompt("Texto", ""), label = new OpenLayers.Feature.Vector(feature);
							label["attributes"] = {
								opacidade : 0.1,
								fillColor : "white",
								strokeWidth : i3GEO.editorOL.simbologia.strokeWidth,
			...
			if (botoes.edita === true && botoes.corta === true && i3GEO.php) {
				i3GEO.editorOL.CortaFeature = new OpenLayers.Control.DrawFeature(i3GEO.desenho.layergrafico, OpenLayers.Handler.Polygon, {
					...
					callbacks : {
							...
							var temp, sel = i3GEO.desenho.layergrafico.selectedFeatures[0].geometry, corta = feature;
			...
			if (botoes.edita === true) {
				i3GEO.editorOL.ModifyFeature = new OpenLayers.Control.ModifyFeature(i3GEO.desenho.layergrafico, {
			...
			if (botoes.selecao === true) {
				i3GEO.editorOL.selbutton = new OpenLayers.Control.SelectFeature(i3GEO.desenho.layergrafico, {
			...
			if (botoes.selecaotudo === true) {
				button = new OpenLayers.Control.Button({
					...
					trigger : function() {
						var fs = i3GEO.desenho.layergrafico.features, n = fs.length, i;
						for (i = 0; i < n; i++) {
			...
			/*
			 * if(botoes.undo===true){ button = new OpenLayers.Control.Button({ displayClass: "editorOLundo", trigger:
			 * function(){i3GEO.editorOL.restauraBackup();}, title: "recupera" }); controles.push(button); adiciona = true; }
			 */
			...
			if (botoes.fecha === true) {
				button = new OpenLayers.Control.Button({
					...
					trigger : function() {
						...
						try {
							YAHOO.editorOL.ferramentas.panel.destroy();
						} catch (e) {
						}
						...
						try {
							YAHOO.salvaGeometrias.container.panel.destroy();
						} catch (e) {
						}
						if (temp === true) {
							try {
		...
		mudaSimbolo : function(estilo, id) {
			var valor = $i(id).value, geos = i3GEO.desenho.layergrafico.selectedFeatures, n = geos.length, i;
			...
			if (!document.getElementById("panelpropriedadesEditor")) {
				...
				var ins =
					"" + '<p class=paragrafo ><b>Estilos (utilize a cor no formato r,g,b):</b></p>'
						+ '<table class=lista7 >'
						+ '	<tr>'
						+ '		<td>Cor do contorno</td><td><input onchange="i3GEO.editorOL.mudaSimbolo(\'strokeColor\',\'i3GEOEditorOLcorContorno\')" type="text" style="cursor:text" id="i3GEOEditorOLcorContorno" size="12" value="'
						+ i3GEO.editorOL.simbologia.strokeColor
						+ '" /></td><td>';
				if (i3GEO.configura) {
					ins +=
						'<img alt="aquarela.gif" style=cursor:pointer src="' + i3GEO.configura.locaplic
							+ '/imagens/aquarela.gif" onclick="i3GEO.util.abreCor(\'\',\'i3GEOEditorOLcorContorno\');" />';
				}
				ins +=
					"" + '		</td>'
						+ '	</tr>'
						+ '	<tr>'
						+ '		<td>Cor do preenchimento</td><td><input onchange="i3GEO.editorOL.mudaSimbolo(\'fillColor\',\'i3GEOEditorOLcorPre\')" type="text" style="cursor:text" id="i3GEOEditorOLcorPre" size="12" value="'
						+ i3GEO.editorOL.simbologia.fillColor
						+ '" /></td><td>';
				if (i3GEO.configura) {
					ins +=
						'<img alt="aquarela.gif" style=cursor:pointer src="' + i3GEO.configura.locaplic
							+ '/imagens/aquarela.gif" onclick="i3GEO.util.abreCor(\'\',\'i3GEOEditorOLcorPre\');" />';
				}
				ins +=
					"" + '		</td>'
						+ '	</tr>'
						+ '	<tr>'
						+ '		<td>Cor da fonte</td><td><input onchange="i3GEO.editorOL.mudaSimbolo(\'fontColor\',\'i3GEOEditorOLcorFonte\')" type="text" style="cursor:text" id="i3GEOEditorOLcorFonte" size="12" value="'
						+ i3GEO.editorOL.simbologia.fontColor
						+ '" /></td><td>';
				if (i3GEO.configura) {
					ins +=
						'<img alt="aquarela.gif" style=cursor:pointer src="' + i3GEO.configura.locaplic
							+ '/imagens/aquarela.gif" onclick="i3GEO.util.abreCor(\'\',\'i3GEOEditorOLcorFonte\');" />';
				}
				ins +=
					"" + '		</td>'
						+ '	</tr>'
						+ '	<tr>'
						+ '		<td>Tamanho da fonte</td><td><input onchange="i3GEO.editorOL.mudaSimbolo(\'fontSize\',\'i3GEOEditorOLfontsize\')" type="text" style="cursor:text" id="i3GEOEditorOLfontsize" size="3" value="' + i3GEO.editorOL.simbologia.fontSize
						+ '" /></td><td></td>'
						+ '	</tr>'
						+ '	<tr>'
						+ '		<td>Opacidade (de 0 a 1)</td><td><input onchange="i3GEO.editorOL.mudaSimbolo(\'opacidade\',\'i3GEOEditorOLopacidade\')" type="text" style="cursor:text" id="i3GEOEditorOLopacidade" size="3" value="'
						+ i3GEO.editorOL.simbologia.opacidade
						+ '" /></td><td></td>'
						+ '	</tr>'
						+ '	<tr>'
						+ '		<td>Largura da linha/contorno</td><td><input onchange="i3GEO.editorOL.mudaSimbolo(\'strokeWidth\',\'i3GEOEditorOLlarguraLinha\')" type="text" style="cursor:text" id="i3GEOEditorOLlarguraLinha" size="2" value="'
						+ i3GEO.editorOL.simbologia.strokeWidth
						+ '" /></td><td></td>'
						+ '	</tr>'
						+ '	<tr>'
		...
		listaGeometrias : function() {
			...
			var g, temp, geos = i3GEO.desenho.layergrafico.features, n = geos.length, ins = "<table class=lista4 >";
			...
			while (n > 0) {
				...
				ins +=
					"<tr><td>" + g.CLASS_NAME
						+ "</td><td style='cursor:pointer;color:blue' onclick='javascript:i3GEO.editorOL.selFeature("
						+ n
						+ ")'>seleciona</td><td style='cursor:pointer;color:blue' onclick='javascript:i3GEO.editorOL.unselFeature("
						+ n
						+ ")'>limpa</td><td style='cursor:pointer;color:blue' onclick='javascript:i3GEO.editorOL.flashFeaturesI("
						+ n
						+ ")'>brilha</td></tr>";
			}
			ins += "</table>";
		...
		ferramentas : function() {
			if (!document.getElementById("panelferramentasEditor")) {
				...
				ins +=
					'</select>' + '<br><br><a class=paragrafo href=# onclick="i3GEO.desenho.layergrafico.destroyFeatures()" >Apaga tudo</a>';
		...
		processageo : function(operacao) {
			...
			var geosel = i3GEO.desenho.layergrafico.selectedFeatures, polis, linhas, pontos, temp;
			if (geosel.length > 0) {
		...
		uniaojts : function(geoms) {
			var n = geoms.length, rwkt = new jsts.io.WKTReader(), wwkt = new jsts.io.WKTWriter(), fwkt = new OpenLayers.Format.WKT(), g, i, uniao;
			if (n > 1) {
		...
		retornaGeometriasTipo : function(features, tipo) {
			var n = features.length, lista = [], i, temp;
			for (i = 0; i < n; i++) {
