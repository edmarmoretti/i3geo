...
i3GEO.janela =
{
	...
	destroi: function () {
		if (i3GEO.janela._formModal) {
			...
			i3GEO.janela._formModal.remove();
			i3GEO.janela._formModal = false;
		}
	},
	...
	formModal: function ({ objJanela = false, idForm = "", expandable = true, resizable = { disabled: true, ghost: true, handles: "se" }, texto = false, footer = false, header = false, onclose = false, backdrop = false, draggable = "enable", css = false } = {}) {
		if (typeof (console) !== 'undefined')
			console.info("i3GEO.janela.formModal()");
		...
		if (!obj) {
			...
			obj.find(".closeModal").on("click", function () {
				i3GEO.janela.destroi();
				if (onclose != false) {
					if (typeof (console) !== 'undefined')
						console.info("onclose janela.formModal");
					onclose();
				}
			});
		}
