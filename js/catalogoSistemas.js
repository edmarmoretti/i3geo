i3GEO.catalogoSistemas = {
		...
		aguarde: function(){
			$("#" + i3GEO.catalogoSistemas.config.idCatalogoNavegacao).text($trad("o1"));
		},
		...
		listaFuncoes: function(id, nome){
			...
			$.each( i3GEO.catalogoSistemas.DADOS[id].FUNCOES, function( i,v ) {
		...
		adiciona : function(w,h,f){
			i3GEO.janela.formModal(
					{
						texto: "<iframe width='100%' hieght='100%' src='" + f + "'></iframe>",
						resizable: {
							disabled: false,
							ghost: true,
							handles: "se,n"
						},
						css: {'cursor': 'pointer', 'width':w, 'height': h,'position': 'fixed','top': 0, 'left': 0, 'right': 0, bottom: 'unset', 'margin': 'auto'}
					});
		}
};